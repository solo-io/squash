...
var _ = Describe("DebugAttachmentClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs DebugAttachments "+test.Description(), func() {
				DebugAttachmentClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func DebugAttachmentClientTest(namespace string, client DebugAttachmentClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
	...
	Eventually(func() DebugAttachmentList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
