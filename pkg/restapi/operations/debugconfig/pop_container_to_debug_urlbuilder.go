...
func (o *PopContainerToDebugURL) WithBasePath(bp string) *PopContainerToDebugURL {
	o.SetBasePath(bp)
	return o
}
...
func (o *PopContainerToDebugURL) Build() (*url.URL, error) {
	...
	var _path = "/debugconfig/platform/containers/{node}/latest"

	node := o.Node
	if node != "" {
		_path = strings.Replace(_path, "{node}", node, -1)
	} else {
		return nil, errors.New("Node is required on PopContainerToDebugURL")
	}
...
func (o *PopContainerToDebugURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PopContainerToDebugURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PopContainerToDebugURL")
	}
