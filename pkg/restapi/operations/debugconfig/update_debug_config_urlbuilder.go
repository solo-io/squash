...
func (o *UpdateDebugConfigURL) Build() (*url.URL, error) {
	...
	var _path = "/debugconfig/{debugConfigId}"

	debugConfigID := o.DebugConfigID
	if debugConfigID != "" {
		_path = strings.Replace(_path, "{debugConfigId}", debugConfigID, -1)
	} else {
		return nil, errors.New("DebugConfigID is required on UpdateDebugConfigURL")
	}
...
func (o *UpdateDebugConfigURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpdateDebugConfigURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpdateDebugConfigURL")
	}
